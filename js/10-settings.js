...
-- Webプッシュ通知のVAPID署名鍵。秘密鍵を含むため誰にも読ませない
-- （RLSを有効にしてポリシーを一切作らない＝service_roleのEdge Functionだけが読める）。
-- 鍵は web-push 関数が初回呼び出し時に自分で生成して書き込むので、人が鍵を扱う必要はない
CREATE TABLE IF NOT EXISTS push_config (
  id smallint PRIMARY KEY DEFAULT 1 CHECK (id = 1),
  public_key text NOT NULL,
  private_jwk jsonb NOT NULL,
  subject text NOT NULL DEFAULT 'https://portergarden.github.io/porter-garden-invoice-app/',
  created_at timestamptz NOT NULL DEFAULT now()
);

-- 端末ごとの通知の宛先。1人が複数の端末（スマホ・PC）を登録できる
CREATE TABLE IF NOT EXISTS push_subscriptions (
  id bigserial PRIMARY KEY,
  user_id text NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  drv_id integer REFERENCES drivers(id) ON DELETE CASCADE,
  endpoint text NOT NULL UNIQUE,   -- ブラウザが発行する送信先URL
  p256dh text NOT NULL,            -- 本文暗号化用の公開鍵
  auth text NOT NULL,              -- 本文暗号化用の共有秘密
  ua text,
  created_at timestamptz NOT NULL DEFAULT now(),
...
CREATE INDEX IF NOT EXISTS push_subscriptions_drv_idx ON push_subscriptions (drv_id);

-- 通知の受け取り設定（種類ごとのオフと、グループごとのミュート）。
-- 絞り込みはEdge Function側で行う（複数人へ一度に送るとき、人ごとの設定を効かせるため）
CREATE TABLE IF NOT EXISTS notify_prefs (
  user_id text PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE,
  chat boolean NOT NULL DEFAULT true,       -- 個別・グループチャット
  board boolean NOT NULL DEFAULT true,      -- 会社からのお知らせ
  statement boolean NOT NULL DEFAULT true,  -- 支払明細書の配信
  muted_group_ids bigint[] NOT NULL DEFAULT '{}',
...
-- 営業所（支店）とコース。同じ取引先で営業所ごと・コースごとに単価が違うときに使う。
-- [{name:'新宿営業所', courses:['朝便','夜便']}, …]  name が空の行は「営業所の区別なし（コースだけ）」
ALTER TABLE clients ADD COLUMN IF NOT EXISTS sites jsonb NOT NULL DEFAULT '[]'::jsonb;  -- 送付時の注意書き（タスク管理の一覧に表示）
...
CREATE TABLE IF NOT EXISTS daily_reports (
  ...
  cli bigint,
  -- 数量は業務の種類ごとに持つ（個人宅配＝個／企業集配＝件と個／チャーター＝件と個。距離は trips[].km）。
...
CREATE TABLE IF NOT EXISTS driver_messages (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  read_at timestamptz,
  deleted_at timestamptz      -- 送信取消。行は残し、本文と添付だけを消す
);
...
--   round（任意）: 'down' なら ◯ごとの端数を切り捨て、'thr' なら thr（対象と同じ単位）以上で切り上げ・未満は切り捨て。無ければ切り上げ
...
-- 営業所・コースも返す（運行入力で選ぶため）。戻り値を変えるときは DROP してから作り直す
DROP FUNCTION IF EXISTS public.list_client_names();
CREATE FUNCTION public.list_client_names()
RETURNS TABLE(id bigint, name text, sites jsonb) LANGUAGE sql SECURITY DEFINER SET search_path TO 'public' AS $function$
  SELECT id, name, COALESCE(sites, '[]'::jsonb) FROM clients ORDER BY name;
$function$;
...
--
-- 誰の登録になるかは、招待を発行したときの宛先で決まる。
--   ・宛先を指定した招待（target_driver_id あり）… その1人にだけ合流する。
--     入力された氏名や電話番号は照合に使わない。URLが第三者に渡っても
--     別のドライバーになりすますことはできない。
--   ・宛先を指定しない招待（新規ドライバー向け）… 電話番号→氏名の順で既存を探し、
--     見つかれば車番を追記、見つからなければ新規に作る。既存へ合流した場合は
--     なりすましの余地が残るため、ログインの自動発行は行わない（created_new=false）。
--
-- 新規作成時はドライバーID(supplier_id)を「既存の数値の最大値+1」で自動採番する
-- （管理側の「＋ドライバー追加」と同じ採番規則。ログインIDにもこの番号が使われる）。
-- 協力会社名が入力された場合は、既存を探して無ければ clients に kind='supplier' で作成して紐づける
-- （会社番号(client_no)も自動採番する。協力会社は取引先マスタへ統合済みで、
--   partner_companies には作らない）
...
-- Webプッシュ: VAPID鍵はポリシーを作らない（service_roleのみ）。
-- 通知の宛先は本人だけが登録・解除でき、他人の端末は見えない
...
CREATE OR REPLACE FUNCTION public.retract_driver_message(p_id bigint)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $function$
DECLARE v_role text; v_drv integer; v_sender text; v_drv_id integer;
BEGIN
  SELECT role, driver_id INTO v_role, v_drv FROM users WHERE auth_uid = auth.uid();
  IF v_role IS NULL THEN RAISE EXCEPTION '権限がありません'; END IF;
  SELECT sender_role, drv_id INTO v_sender, v_drv_id FROM driver_messages WHERE id = p_id;
  IF v_sender IS NULL THEN RAISE EXCEPTION 'メッセージが見つかりません'; END IF;
  IF v_role NOT IN ('admin','editor')
     AND NOT (v_sender = 'driver' AND v_drv IS NOT NULL AND v_drv = v_drv_id) THEN
    RAISE EXCEPTION '自分が送ったメッセージだけ取り消せます';
  END IF;
  UPDATE driver_messages
     SET deleted_at = now(), body = NULL,
         file_path = NULL, file_name = NULL, file_type = NULL, file_size = NULL
   WHERE id = p_id;
END;
$function$;
REVOKE EXECUTE ON FUNCTION public.retract_driver_message(bigint) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.retract_driver_message(bigint) TO authenticated;

CREATE OR REPLACE FUNCTION public.retract_chat_group_message(p_id bigint)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $function$
DECLARE v_role text; v_drv integer; v_uid text; v_key text;
BEGIN
  SELECT role, driver_id, id INTO v_role, v_drv, v_uid FROM users WHERE auth_uid = auth.uid();
  IF v_role IS NULL THEN RAISE EXCEPTION '権限がありません'; END IF;
  SELECT sender_key INTO v_key FROM chat_group_messages WHERE id = p_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'メッセージが見つかりません'; END IF;
  IF v_role NOT IN ('admin','editor')
     AND NOT (v_drv IS NOT NULL AND v_key = 'driver:' || v_drv) THEN
    RAISE EXCEPTION '自分が送ったメッセージだけ取り消せます';
  END IF;
...
END;
$function$;
REVOKE EXECUTE ON FUNCTION public.retract_chat_group_message(bigint) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.retract_chat_group_message(bigint) TO authenticated;
